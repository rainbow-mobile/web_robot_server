...
const external_control_dto_1 = require("../../apis/control/dto/external.control.dto");
const isEqual = (a, b) => {
    return JSON.stringify(a) === JSON.stringify(b);
};
let SocketGateway = class SocketGateway {
    constructor(networkService, logService, moveRepository) {
    ...
    setConnectChecker() {
        this.connectChecker = setTimeout(() => {
            ...
            if (!this.externalAccessory) {
                socket_logger_1.default.error(`[CHECKER] connect Checker : ExternalAccessory not connected`);
            }
            else {
                socket_logger_1.default.debug(`[CHECKER] connect Checker : ExternalAccessory connected`);
            }
            if (!this.acs) {
    ...
    handleConnection(client) {
        ...
        if (client.handshake.query.name == 'slamnav') {
            ...
            if (this.externalAccessory) {
                socket_logger_1.default.warn(`[CONNET] externalAccessory already connected -> ignored `);
            }
            else {
        ...
        if (client.handshake.query.name == 'slamnav') {
            ...
            if (this.externalAccessory) {
                if (this.externalAccessory.id === client.id) {
                    ...
                    socket_logger_1.default.warn(`[CONNECT] externalAccessory disconnected -> another one. ignored`);
                }
            }
        }
        else if (client.handshake.query.name == 'taskman') {
    ...
    async handleFootSTatusMessage(payload, client) {
        try {
            if (client.id == this.externalAccessory?.id) {
                if (payload == null || payload == undefined) {
                    socket_logger_1.default.warn(`[STATUS] footStatus: NULL`);
                    return;
                }
                ...
                if (this.slamnav) {
                    this.slamnav.emit('footStatus', (0, network_util_1.stringifyAllValues)(json));
                }
                else {
                }
                if (this.frsSocket?.connected) {
                ...
                socket_logger_1.default.warn(`[STATUS] another externalAccessory footStatus ${this.externalAccessory?.id}, ${client.id}`);
            }
        }
        catch (error) {
            socket_logger_1.default.error(`[STATUS] footStatus : ${(0, error_util_1.errorToJson)(error)}`);
        }
    }
    parseStatus(status) {
        try {
            if (status.power) {
                if (status.power.tabos_status) {
                    status.power.tabos_status = this.parseTabosStatus(parseInt(status.power.tabos_status));
                }
            }
            return status;
        }
    ...
    async handleStatusMessage(payload, client) {
        try {
            if (client.id == this.slamnav?.id) {
                ...
                if (this.lastStatus?.robot_state?.localization !==
    ...
    async startAlarmCode(alarmCode) {
        this.setAlarm({ alarmCode: alarmCode.toString(), state: true });
        const alarmEntity = await this.logService.getAlarmDetail(alarmCode);
        (0, equipment_logger_1.setAlarmGeneralLog)(alarmEntity, equipment_enum_1.GeneralOperationStatus.START);
    }
    async clearAlarmCode(alarmCode) {
        this.clearAlarm({ alarmCode: alarmCode.toString(), state: false });
        const alarmEntity = await this.logService.getAlarmDetail(alarmCode);
        (0, equipment_logger_1.setAlarmGeneralLog)(alarmEntity, equipment_enum_1.GeneralOperationStatus.END);
    }
    async clearAlarm(data) {
        try {
            ...
            if (lastAlarm) {
                if (data.alarmCode === lastAlarm.alarmCode &&
                    data.state === lastAlarm.state) {
                    socket_logger_1.default.warn(`[LOG] duplicate alarm : ${data.alarmCode} -> ${alarmDetail.alarmDescription}`);
                    return;
                }
            }
            socket_logger_1.default.info(`[LOG] clear alarm : ${data.alarmCode} -> ${alarmDetail.alarmDescription}`);
            const alarmDto = {
            ...
            socket_logger_1.default.error(`[LOG] clearAlarm : ${(0, error_util_1.errorToJson)(error)}`);
            if (error instanceof microservices_1.RpcException)
                throw error;
            throw new microservices_1.RpcException('서버에 에러가 발생했습니다.');
        }
    ...
    async handleExternalResponseMessage(payload) {
        try {
            if (payload == null || payload == undefined) {
                socket_logger_1.default.warn(`[RESPONSE] externalResponse: NULL`);
                return;
            }
            const json = JSON.parse(payload);
            this.server
            ...
            if (json.command === external_control_dto_1.FootCommand.Move) {
                if (json.result === 'accept') {
                    (0, equipment_logger_1.generateGeneralLog)({
                        logType: equipment_enum_1.GeneralLogType.AUTO,
                        status: equipment_enum_1.GeneralStatus.RUN,
                        scope: equipment_enum_1.GeneralScope.FOOT,
                        operationName: equipment_enum_1.FootOperationName.READY,
                        operationStatus: 'END',
                    });
                    (0, equipment_logger_1.generateGeneralLog)({
                        logType: equipment_enum_1.GeneralLogType.AUTO,
                        status: equipment_enum_1.GeneralStatus.RUN,
                        scope: equipment_enum_1.GeneralScope.FOOT,
                        operationName: equipment_enum_1.FootOperationName.MOVE,
            ...
            socket_logger_1.default.debug(`[RESPONSE] externalResponse : ${JSON.stringify(json)}`);
        }
        catch (error) {
            socket_logger_1.default.error(`[RESPONSE] externalResponse : ${(0, error_util_1.errorToJson)(error)}`);
            throw error();
        }
    }
...
__decorate([
    (0, websockets_1.SubscribeMessage)('footStatus'),
    __param(0, (0, websockets_1.MessageBody)()),
    __param(1, (0, websockets_1.ConnectedSocket)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, socket_io_1.Socket]),
    __metadata("design:returntype", Promise)
], SocketGateway.prototype, "handleFootSTatusMessage", null);
__decorate([
...
__decorate([
    (0, websockets_1.SubscribeMessage)('externalResponse'),
    __param(0, (0, websockets_1.MessageBody)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], SocketGateway.prototype, "handleExternalResponseMessage", null);
__decorate([
...
exports.SocketGateway = SocketGateway = __decorate([
    ...
    __param(2, (0, typeorm_1.InjectRepository)(move_entity_1.MoveLogEntity)),
