...
let UploadController = class UploadController {
    ...
    async uploadMap(data, res) {
        const path = (0, path_1.join)((0, os_1.homedir)(), 'maps', data.mapNm);
        const path2 = (0, path_1.join)(this.dataBasePath, 'maps', data.mapNm);
        const zipFileName = `${data.name}.zip`;
        let originalFilePath;
        let zipFilePath;
        if (fs.existsSync(path2)) {
            originalFilePath = path2;
            zipFilePath = (0, path_1.join)(path2, zipFileName);
        }
        else {
            originalFilePath = path;
            zipFilePath = (0, path_1.join)(path, zipFileName);
        }
        try {
    ...
    async publishedMap(req, mapNm, res) {
        (0, upload_middleware_1.uploadMiddleware)(req, res, async (err) => {
            ...
            try {
                ...
                const zipFilePath = (0, path_1.join)((0, os_1.homedir)(), 'upload', req.file.originalname);
                const path = (0, path_1.join)((0, os_1.homedir)(), 'maps', mapNm);
                const path2 = (0, path_1.join)(this.dataBasePath, 'maps', mapNm);
                let extractToPath;
                if (fs.existsSync(path2)) {
                    extractToPath = path2;
                }
