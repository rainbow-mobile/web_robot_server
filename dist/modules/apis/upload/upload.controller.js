...
let UploadController = class UploadController {
    ...
    async uploadMap(data, res) {
        ...
        const originalFilePath = (0, path_1.join)('/data/maps', data.mapNm);
        const zipFilePath = (0, path_1.join)(originalFilePath, zipFileName);
        try {
    ...
    async publishedMap(req, mapNm, res) {
        (0, upload_middleware_1.uploadMiddleware)(req, res, async (err) => {
            ...
            try {
                ...
                const zipFilePath = (0, path_1.join)('data', 'upload', req.file.originalname);
                const extractToPath = (0, path_1.join)('/data/maps', mapNm);
                ...
                fs.unlink('/data/upload/' + req.file.originalname, (err) => {
                    if (err)
                        http_logger_1.default.error(`[UPLOAD] PublishMap: Delete Zip ('/data/upload' + req.file.originalname}) ${(0, error_util_1.errorToJson)(err)}`);
                    http_logger_1.default.info(`[UPLOAD] PublishMap: Delete Zip ('/data/upload' + req.file.originalname})`);
                });
            }
        });
    }
