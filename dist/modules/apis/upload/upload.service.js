    ...
    async downloadMap(fileName) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const fileStream = fs.createWriteStream((0, os_1.homedir)() + "/maps/" + fileName);
                response.data.pipe(fileStream);
                fileStream.on("finish", async () => {
                    http_logger_1.default.info(`[UPLOAD] DownloadMap: Done`);
                    const zipFilePath = path.join((0, os_1.homedir)(), "maps", fileName);
                    const extractToPath = path.join((0, os_1.homedir)(), "maps");
                    ...
                    fs.unlink((0, os_1.homedir)() + "/maps/" + fileName, (err) => {
                        if (err)
