...
const external_control_dto_1 = require("./dto/external.control.dto");
let ControlController = class ControlController {
    ...
    async externalCommand(request, res) {
        try {
            http_logger_1.default.info(`[COMMAND] externalCommand Control`);
            const response = await this.controlService.externalCommand(request);
            res.send(response);
        }
        catch (error) {
            http_logger_1.default.error(`[COMMAND] externalCommand Control: ${error.status} -> ${(0, error_util_1.errorToJson)(error.data)}`);
            return res.status(error.status).send(error.data);
        }
    }
};
...
__decorate([
    ...
    __param(1, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [external_control_dto_1.ExternalCommandDto, Object]),
    __metadata("design:returntype", Promise)
], ControlController.prototype, "externalCommand", null);
