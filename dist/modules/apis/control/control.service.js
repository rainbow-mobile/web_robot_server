...
let ControlService = class ControlService {
    ...
    async externalCommand(request) {
        return new Promise((resolve, reject) => {
            if (this.socketGateway.externalAccessory != null) {
                const data = { ...request, time: Date.now().toString() };
                this.socketGateway.externalAccessory.emit('externalCommand', (0, network_util_1.stringifyAllValues)(data));
                http_logger_1.default.info(`[CONTROL] externalCommand: ${JSON.stringify(data)}`);
                this.socketGateway.externalAccessory.once('externalResponse', (resp) => {
                    http_logger_1.default.info(`[CONTROL] externalResponse: ${JSON.stringify(resp)}`);
                    resolve(resp);
                    clearTimeout(timeoutId);
                });
                const timeoutId = setTimeout(() => {
