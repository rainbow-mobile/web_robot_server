...
let SettingService = class SettingService {
    ...
    async saveSetting(type, data) {
        try {
            ...
            return await (0, file_util_1.saveJson)(Path.join((0, os_1.homedir)(), 'slamnav2', 'config', type, 'config.json'), newData);
        }
        catch (error) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (fs.existsSync(dir)) {
                    reject({ status: common_1.HttpStatus.CONFLICT, data: { message: http_status_messages_constants_1.HttpStatusMessagesConstants.FILE.DUPLICATE_WRITE } });
                }
                else {
                    const tempData = {
                        LIMIT_V: "1.0",
                ...
                reject({ status: common_1.HttpStatus.INTERNAL_SERVER_ERROR, data: { message: http_status_messages_constants_1.HttpStatusMessagesConstants.INTERNAL_SERVER_ERROR_500 } });
            }
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (error.code == 'ENOENT') {
                    reject({ status: common_1.HttpStatus.NOT_FOUND, message: http_status_messages_constants_1.HttpStatusMessagesConstants.FILE.NOT_FOUND_404 });
                }
                else {
                    reject({ status: common_1.HttpStatus.INTERNAL_SERVER_ERROR, message: http_status_messages_constants_1.HttpStatusMessagesConstants.FILE.FAIL_READ_500 });
                }
            }
        });
    }
