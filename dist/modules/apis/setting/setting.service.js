...
let SettingService = class SettingService {
    ...
    async settingRequest(dto) {
        return new Promise(async (resolve, reject) => {
            if (this.socketGateway.slamnav != null) {
                http_logger_1.default.info(`[SETTING] settingRequest: ${JSON.stringify(dto)}`);
                this.socketGateway.slamnav.emit('settingRequest', dto);
                this.socketGateway.slamnav.once('settingResponse', (data) => {
                    const json = JSON.parse(data);
                    http_logger_1.default.info(`[SETTING] settingResponse: ${JSON.stringify(json)}, ${json.result}`);
            ...
            if (this.socketGateway.slamnav != null) {
                ...
                this.socketGateway.slamnav.once('cameraInfoResponse', (res) => {
                    ...
                    if (parsedRes.status === '200') {
                        http_logger_1.default.info(`[Setting] cameraInfoResponse: ${JSON.stringify(res)}`);
                        resolve(res);
                    }
                    else {
                        reject(new common_1.BadRequestException('카메라 정보를 가져오는데 실패했습니다'));
                    }
                    clearTimeout(timeoutId);
                });
                const timeoutId = setTimeout(() => {
        ...
        return new Promise((resolve, reject) => {
            if (this.socketGateway.slamnav != null) {
                ...
                this.socketGateway.slamnav.once('cameraOrderChangeResponse', (res) => {
                    http_logger_1.default.info(`[Setting] cameraOrderChange Response: ${JSON.stringify(res)}`);
                    http_logger_1.default.info(`[Setting] cameraOrderChange Response:  status: ${JSON.stringify(res.status)}, ${res.status === '200'}`);
                    const parsedRes = JSON.parse(res);
                    if (parsedRes.status === '200') {
                        resolve(res);
                    }
                    else {
                        reject(new common_1.BadRequestException('카메라 순서 변경에 실패했습니다'));
                    }
                    clearTimeout(timeoutId);
                });
                const timeoutId = setTimeout(() => {
