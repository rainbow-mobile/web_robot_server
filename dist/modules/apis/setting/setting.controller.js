...
let SettingController = class SettingController {
    ...
    async getSetting(type, res) {
        try {
            if (type == "") {
                return res.status(common_1.HttpStatus.BAD_REQUEST).send({ message: http_status_messages_constants_1.HttpStatusMessagesConstants.INVALID_DATA_400 });
            }
    ...
    async savePreset(type, id, data, res) {
        try {
            ...
            if (isNaN(Number(data.LIMIT_V)) || isNaN(Number(data.LIMIT_V_ACC)) || isNaN(Number(data.LIMIT_W)) ||
                isNaN(Number(data.LIMIT_W_ACC)) || isNaN(Number(data.LIMIT_PIVOT_W)) || isNaN(Number(data.ED_V)) ||
                isNaN(Number(data.DRIVE_EPS)) || isNaN(Number(data.DRIVE_H)) || isNaN(Number(data.DRIVE_K)) ||
                isNaN(Number(data.DRIVE_L)) || isNaN(Number(data.DRIVE_T)) ||
                isNaN(Number(data.ST_V)) || isNaN(Number(data.DRIVE_A)) || isNaN(Number(data.DRIVE_B))) {
                return res.status(common_1.HttpStatus.BAD_REQUEST).send({ message: http_status_messages_constants_1.HttpStatusMessagesConstants.INVALID_DATA_400 });
            }
