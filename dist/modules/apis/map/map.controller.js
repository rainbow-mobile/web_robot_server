...
let MapController = class MapController {
    ...
    async getTilesExist(mapNm) {
        try {
            ...
            const path = (0, path_1.join)((0, os_1.homedir)(), 'maps', mapNm, 'tiles');
            const path2 = (0, path_1.join)(dataBasePath, 'maps', mapNm, 'tiles');
    ...
    async getTiles(mapNm, z, y, x, res) {
        try {
            ...
            const dataBasePath = this.configService.get('dataBasePath');
            const path = (0, path_1.join)((0, os_1.homedir)(), 'maps', mapNm, 'tiles', z, x, y + '.png');
            const path2 = (0, path_1.join)(dataBasePath, 'maps', mapNm, 'tiles', z, x, y + '.png');
            if (fs.existsSync(path)) {
    ...
    async deleteMap(mapNm, res) {
        try {
            http_logger_1.default.debug(`[MAP] deleteMap: ${mapNm}`);
            const response = await this.mapService.deleteMap(mapNm);
            res.send(response);
        }
        catch (error) {
            http_logger_1.default.error(`[MAP] deleteMap ${mapNm}: ${error.status} -> ${JSON.stringify(error.data)}`);
            return res.status(error.status).send(error.data);
        }
    }
};
...
__decorate([
    (0, common_1.Delete)('delete/:mapNm'),
    (0, swagger_1.ApiOperation)({
        summary: '맵 삭제',
    ...
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], MapController.prototype, "deleteMap", null);
