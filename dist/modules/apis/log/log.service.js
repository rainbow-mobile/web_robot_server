...
let LogService = class LogService {
    constructor(statusRepository, systemRepository, alarmLogRepository, alarmRepository, dataSource) {
    ...
    async writeAlarmLog(alarmCode, alarmDetail, state) {
        try {
    ...
    async getAlarmDetail(alarmCode) {
        ...
        if (result.length > 0) {
            ...
            throw new microservices_1.RpcException(`Not Found alarmCode ${alarmCode}`);
        }
    }
    ...
    async resetAlarms() {
        try {
            ...
            http_logger_1.default.error(`[LOG] resetAlarms Error : ${(0, error_util_1.errorToJson)(error)}`);
            throw new microservices_1.RpcException(`알람로그를 삭제할 수 없습니다.`);
        }
    }
    async getAlarmsAll() {
        try {
            ...
            http_logger_1.default.error(`[LOG] getAlarmsAll Error : ${(0, error_util_1.errorToJson)(error)}`);
            throw new microservices_1.RpcException(`알람로그를 읽을 수 없습니다.`);
        }
    }
    async getAlarms() {
        try {
            ...
            http_logger_1.default.error(`[LOG] getAlarms Error : ${(0, error_util_1.errorToJson)(error)}`);
            throw new microservices_1.RpcException(`알람로그를 읽을 수 없습니다.`);
        }
    }
    async getLastAlarm(alarmCode) {
        try {
            const allLogs = await this.alarmLogRepository.find({
                order: { time: 'DESC' },
                where: { alarmCode: alarmCode }
            });
            const latestLogsByCode = new Map();
    ...
    async setAlarmsFlag(list) {
        try {
            ...
            http_logger_1.default.error(`[LOG] setAlarmsFlag Error : ${(0, error_util_1.errorToJson)(error)}`);
            throw new microservices_1.RpcException(`알람로그를 저장할 수 없습니다.`);
        }
    }
    async getAlarms2(param) {
        try {
    ...
    async setAlarm(param) {
        try {
            if (param.alarmCode === undefined || param.alarmCode === "") {
                throw new microservices_1.RpcException('alarmCode 값이 없습니다.');
            }
            if (param.state === undefined) {
                throw new microservices_1.RpcException('state 값이 없습니다.');
            }
            this.alarmLogRepository.save(param);
        }
        catch (error) {
            http_logger_1.default.error(`[LOG] setAlarm Error : ${(0, error_util_1.errorToJson)(error)}`);
            throw new microservices_1.RpcException('알람을 저장할 수 없습니다.');
        }
    }
    async readGeneralLog(dir) {
        try {
            ...
            if (fs.openSync(dir, "r")) {
                ...
                throw new microservices_1.RpcException('파일이 없습니다.');
            }
        }
        catch (error) {
            http_logger_1.default.error(`[FILE] readJson: ${dir}, ${(0, error_util_1.errorToJson)(error)}`);
            throw new microservices_1.RpcException('로그를 읽을 수 없습니다.');
        }
    }
    async getLogs(type, param) {
        try {
            const logPath = (0, os_1.homedir)() + '/log/' + type;
            const logdata = [];
    ...
    async archiveOldDBData(type, date) {
        ...
        if (oldData.length > 0) {
            const archiveDir = path.join((0, os_1.homedir)(), 'log', 'archive', type);
    ...
    async archiveOldJSONData(type, date) {
        const filePath = (0, os_1.homedir)() + '/log/' + type + '/' + date + '.log';
        if (fs.existsSync(filePath)) {
            const zipPath = path.join((0, os_1.homedir)(), 'log', 'archive', type, date + '.archive');
            const zip = new AdmZip();
            zip.addLocalFile(filePath);
...
exports.LogService = LogService = __decorate([
    ...
    __param(2, (0, typeorm_1.InjectRepository)(alarmlog_entity_1.AlarmLogEntity)),
    __param(3, (0, typeorm_1.InjectRepository)(alarm_entity_1.AlarmEntity)),
