...
let TaskController = class TaskController {
    ...
    async loadTask(mapName, taskName, res) {
        try {
            ...
            const data = await this.taskService.loadTask(os.homedir() + '/maps/' + mapName + '/' + taskName);
    ...
    async runTask(res) {
        try {
            ...
            return res.status(common_1.HttpStatus.ACCEPTED).send({ message: '성공적으로 요청했습니다' });
        }
        catch (error) {
    ...
    async readTaskList(mapName, res) {
        try {
            ...
            const tasks = await this.taskService.getTaskList(os.homedir() + '/maps/' + mapName);
    ...
    async saveTask(data, mapName, taskName, res) {
        try {
            ...
            const task = await this.taskService.save(os.homedir() + "/maps/" + mapName + "/" + taskName, data.data);
            return res.send(task);
        }
        catch (error) {
