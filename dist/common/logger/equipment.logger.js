...
function getCustomFilenameWithDate(suffix) {
    ...
    const baseLogDir = path.join('C:/data', 'log', 'samsung-em');
...
function generateGeneralLog(param) {
    const header = 'SEM_LOG_VERSION=2.0\nDateTime\tMachineID\tLogType\tLotID\tRecipe\tProductID\tStatus\tScope\tOperationName\tOperationStatus\tData';
    if (param.operationStatus === equipment_enum_1.GeneralOperationStatus.END) {
        if (!lastGeneralLog ||
            lastGeneralLog.operationStatus !== equipment_enum_1.GeneralOperationStatus.START ||
            lastGeneralLog.operationName !== param.operationName) {
            socket_logger_1.default.warn(`[LOG] generateGeneralLog : unknown END (${param.operationName}, ${param.operationStatus}) (${lastGeneralLog?.operationName}, ${lastGeneralLog?.operationStatus})`);
            return false;
        }
    }
    const row = [
