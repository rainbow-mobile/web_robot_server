...
async function getState(state){
    if(state.state.charge == "true"){
        ...
        if(state.state.power == "false"){
            ...
            if(state.state.map == "" || state.state.localization != "good" || state.motor[0].status != 1
...
async function updateState(state){
    try{
        var sql = "INSERT state (state, auto_state, localization, power, emo, obs_state, charging, inlier_ratio, inlier_error) values (";
