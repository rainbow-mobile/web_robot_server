...
export class ControlController {
  ...
  async mappingStart(@Res() res: Response) {
    try {
      ...
      httpLogger.error(
        `[COMMAND] mapping start: ${error.status} -> ${errorToJson(error.data)}`,
      );
  ...
  async mappingStop(@Res() res: Response) {
    try {
      ...
      httpLogger.error(
        `[COMMAND] mapping stop: ${error.status} ->${errorToJson(error.data)}`,
      );
      res.status(error.status).send(error.data);
    }
  }
  ...
  async mappingSave(@Param('name') name: string, @Res() res: Response) {
    try {
      ...
      httpLogger.error(
        `[COMMAND] mapping save: ${name}, ${error.status} -> ${errorToJson(error.data)}`,
      );
  ...
  async mappingReload(@Res() res: Response) {
    try {
      ...
      httpLogger.error(
        `[COMMAND] mapping reload: ${error.status} -> ${errorToJson(error.data)}`,
      );
      res.status(error.status).send(error.data);
    }
  }
  ...
  async dockStart(@Res() res: Response) {
    try {
      ...
      httpLogger.error(
        `[COMMAND] dock start: ${error.status} -> ${errorToJson(error.data)}`,
      );
      res.status(error.status).send(error.data);
    }
  }
  ...
  async dockStop(@Res() res: Response) {
    try {
      ...
      httpLogger.error(
        `[COMMAND] undock start: ${error.status} -> ${errorToJson(error.data)}`,
      );
      res.status(error.status).send(error.data);
    }
  }
    ...
    try {
      ...
      if (data.command == 'init') {
        if (data.x == '' || data.y == '' || data.rz == '') {
          httpLogger.warn(
            `[COMMAND] Localization Parameter Missing : x, y, rz`,
          );
          return res
            .status(HttpStatus.BAD_REQUEST)
        ...
        httpLogger.warn(
          `[COMMAND] Localization Command Unknown : ${data.command}`,
        );
        return res
          .status(HttpStatus.BAD_REQUEST)
      ...
      httpLogger.error(
        `[COMMAND] localization: ${error.status} -> ${errorToJson(error.data)} ${JSON.stringify(data)}`,
      );
      return res.status(error.status).send(error.data);
    }
  }
  ...
  async lidarControl(@Body() data: LidarControlDto, @Res() res: Response) {
    try {
      httpLogger.info(
        `[COMMAND] Lidar Control : ${data.command}, ${data.frequency}`,
      );
      const response = await this.controlService.sendCommand(
        'lidarOnOff',
        data,
      );
      res.send(response);
      ...
      httpLogger.error(
        `[COMMAND] Lidar Control: ${error.status} -> ${errorToJson(error.data)} ${JSON.stringify(data)}`,
      );
      return res.status(error.status).send(error.data);
    }
  }
  ...
  async pathControl(@Body() data: LidarControlDto, @Res() res: Response) {
    try {
      httpLogger.info(
        `[COMMAND] Path Control : ${data.command}, ${data.frequency}`,
      );
      const response = await this.controlService.sendCommand('pathOnOff', data);
      res.send(response);
    } catch (error) {
      httpLogger.error(
        `[COMMAND] Path Control: ${error.status} -> ${errorToJson(error.data)} ${JSON.stringify(data)}`,
      );
      return res.status(error.status).send(error.data);
    }
  }
