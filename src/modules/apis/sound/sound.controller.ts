...
export class SoundController {
  ...
  @ApiOperation({
    summary: '사운드 파일 추가',
    description:
      '경로 내 사운드 파일 추가 (form-data file에 파일(mp3)넣어서 POST)',
  })
  async addSoundFile(@Req() req: Request, @Res() res: Response) {
    soundMiddleware(req, res, async (err) => {
      ...
      try {
        httpLogger.info(
          `[SOUND] addSoundFile: Download Done ${req.file.originalname}`,
        );
        res.status(HttpStatus.CREATED).send({
          message: HttpStatusMessagesConstants.SUCCESS_201,
