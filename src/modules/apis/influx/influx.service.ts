...
export class InfluxDBService {
  ...
  async writeMoveStatus(status: StatusPayload) {
    try {
      ...
      const result = await this.client.write(
        [posePoint, moveStatePoint, goalPoint, curPoint],
        this.database,
        '',
        { precision: 'ns' },
      );
  ...
  async writeStatus(status: StatusPayload) {
    try {
      ...
      const powerPoint = Point.measurement('power')
        ...
        .setFloatField(
          'charge_current',
          parseFloat(status.power.charge_current),
        )
        .setFloatField(
          'contact_voltage',
          parseFloat(status.power.contact_voltage),
        );
      const conditionPoint = Point.measurement('condition')
        .setTag('type', type)
        .setFloatField(
          'inlier_ratio',
          parseFloat(status.condition.inlier_ratio),
        )
        .setFloatField(
          'inlier_error',
          parseFloat(status.condition.inlier_error),
        )
        .setFloatField(
          'mapping_ratio',
          parseFloat(status.condition.mapping_ratio),
        )
        .setFloatField(
          'mapping_error',
          parseFloat(status.condition.mapping_error),
        );
  ...
  async queryData(database: string, table: string) {
    return new Promise(async (resolve, reject) => {
      try {
        const result = this.client.queryPoints(
          `SELECT * FROM ${table}`,
          database,
        );
        const array = [];
