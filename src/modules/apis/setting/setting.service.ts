...
export class SettingService {
    ...
    async getSetting(type: string){
        const data = await readJson(Path.join(homedir(),'slamnav2','config',type,'config.json'));
    ...
    async deletePreset(type: string, id: string){
        return await deleteFile(Path.join(homedir(),'slamnav2','config',type,'preset_'+id+'.json'));
    }

    async makePreset(type: string, id: string){
        return new Promise(async (resolve, reject) => {
            try{
                const dir = Path.join(homedir(),'slamnav2','config',type,'preset_'+id+'.json');
                if(fs.existsSync(dir)){
                    reject({status:HttpStatus.CONFLICT,data:{message:HttpStatusMessagesConstants.FILE.DUPLICATE_WRITE}})
                }else{
                    const tempData = {
                        LIMIT_V: "1.0",
                        LIMIT_W: "45.0",
    ...
    async savePreset(type: string, id: string, data:PresetDto){
        return new Promise(async (resolve, reject) => {
            try{
                const fileData = await readJson(Path.join(homedir(),'slamnav2','config',type,'preset_'+id+'.json'));
                const mergeData = await this.deepMerge(fileData, data);
        ...
        return new Promise(async (resolve, reject) => {
            try {
              ...
              if(error.code == 'ENOENT'){
                ...
                reject({status:HttpStatus.INTERNAL_SERVER_ERROR, message:HttpStatusMessagesConstants.FILE.FAIL_READ_500});
              }
            }
          });
    }
