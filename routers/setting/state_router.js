...
router.get('/log/power/battery', (req,res) =>{
    try{
        ...
        const addDisconForGaps = (filteredArray) => {
            ...
            for (let i = 0; i < filteredArray.length; i++) {
                result.push({time:new Date(moment(filteredArray[i].time)),value:filteredArray[i].battery_out});
                if (i < filteredArray.length - 1) {
                    if (calculateGapTime(filteredArray[i].time,filteredArray[i + 1].time) > 20) { // 20초 이상 공백이 있을 때
                        const disconEntry = {
        ...
        const finalArray = addDisconForGaps(data.map(({time,battery_out}) => ({time,battery_out})));
        res.send(finalArray);
        })
    }catch(error){
        console.error("SelectStateAll Error : ", error);
